<script>
  ...
  const UNSPLASH_ACCESS_KEY =
    "525c4e686da1895f3796d3d9a5aecda0eab5d9910b8bbda85713e19b5ea3a8ec";
  ...
  const loadMoreResults = entries => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) searchUnsplash();
    });
  };
  ...
  function searchUnsplash() {
    ...
    fetch(endpoint)
      ...
      .finally(() => {
        ...
        if (nextPage >= Number(totalPages)) observer.unobserve(target);
      });
  }
</script>
